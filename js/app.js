...
// Edit task
export const modifyTask = (divIndex, taskIndex) => {
    const taskManager = new TaskManager(getTasks());
...
export const updateTask = (event, index) => {
    const taskTitle = document.querySelector('#modifyTaskForm #taskTitle');
    const taskDescription = document.querySelector('#modifyTaskForm #taskDescription');
    const taskDueDate = document.querySelector('#modifyTaskForm #taskDueDate');
    const taskPriority = document.querySelector('#modifyTaskForm #taskPriority');
    const taskCategory = document.querySelector('#modifyTaskForm #taskCategory');
    const taskIndex = document.querySelector('#modifyTaskForm #taskIndex');
    event.preventDefault();
    if (!taskTitle || !taskDescription || !taskDueDate || !taskPriority || !taskCategory)
        return;
    const taskManager = new TaskManager(getTasks());
    const task = {
...
// Appelle la fonction principale lorsque le contenu de la page est chargé
