...
export const displayTasks = (tasks: Task[]): void => {
  ...
  tasks.forEach((task: Task, index: number) => {
    ...
    taskDiv.innerHTML = `
      ...
      <button class="edit-btn" onclick="modifyTask(${index}, ${taskIndex})">Modifier</button>
    `;
...
export const  modifyTask = (divIndex: number, taskIndex: number): void => {
  const taskManager = new TaskManager(getTasks());
  ...
  taskDiv.innerHTML = `
    ...
    <form id="modifyTaskForm" onsubmit="updateTask(event, ${taskIndex})">
...
export const updateTask = (event: Event, index: number): void => {
  const taskTitle: HTMLInputElement | null = document.querySelector('#modifyTaskForm #taskTitle');
  const taskDescription: HTMLTextAreaElement | null = document.querySelector('#modifyTaskForm #taskDescription');
  const taskDueDate: HTMLInputElement | null = document.querySelector('#modifyTaskForm #taskDueDate');
  const taskPriority: HTMLSelectElement | null = document.querySelector('#modifyTaskForm #taskPriority');
  const taskCategory: HTMLSelectElement | null = document.querySelector('#modifyTaskForm #taskCategory');
  const taskIndex: HTMLInputElement | null = document.querySelector('#modifyTaskForm #taskIndex');

event.preventDefault();

if (!taskTitle || !taskDescription || !taskDueDate || !taskPriority || !taskCategory) return;
const taskManager = new TaskManager(getTasks());
const task: Task = {
...
taskManager.modifyTask(task.id, task);
localStorage.setItem('tasks', JSON.stringify(taskManager.tasks));
displayTasks(taskManager.tasks);
  displayCategories();
}
...
export const displayCategories = (): void => {
  ...
  const filterCategory: HTMLSelectElement | null = document.querySelector('#filterCategory');
    if (filterCategory) {
        ...
        ${categories.map((category: ICategory) => `<option value="${category.id}">${category.name}</option>`).join('')}
        `;
    }
}
